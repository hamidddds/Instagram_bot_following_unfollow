...
class Following:
    ...
    def Following(self):
        V = py.locateOnScreen('images/like_button.png',
                              region=(700, 800, 600, 400), confidence=0.8)  # bazi pageha like baste hast bayad shekle dige ham bezari
        if V == None:
            ChangeTheProcessBar('Cannot Recognize The Target Post Page???')
            print('Cannot Recognize The Target Post Page???')
            return self.Followed, self.status, self.FlagEndOfPost
        ...
        # if the above expresion failed so this get activated

        Others = py.locateCenterOnScreen('images/others.png',
                                         region=(1000, 800, 600, 200), confidence=0.7)

        if (Others != None) and (self.Following_page_validity() != True):
            HumanLikeMove(Others[0], Others[1])
            py.click()
            time.sleep(1)

        if self.Following_page_validity() != True:
            ChangeTheProcessBar('Cannot Recognize The following???')
            EnterUrl('www.instagram.com')
            return self.Followed, 'NotDone', FlagEndOfPost

        while self.Followed < self.Following_Number:
            ...
            else:  # chon validity check shode mishe in karo kard
                im = ImageGrab.grab(bbox=(765, 377, 1130, 755))
                time.sleep(random.uniform(0.8, 1.2))
                HumanLikeMove(1000+random.randint(-10, 10),
                              550+random.randint(-20, 20),)
            ...
            self.EndOfScroll = py.locateOnScreen(im, confidence=0.9)
            if self.EndOfScroll != None:
                FlagEndOfPost = 1
                print('End of scroll,lets go to the next post!')
                ChangeTheProcessBar('End of scroll,lets go to the next post!')
                return self.Followed, 'Done', FlagEndOfPost
    ...
    def following(self):
        ...
        while 1:
            ...
            if Homepage == Name_Of_page:
                return username
            username.append(Name_Of_page.split(
